Then(/^it should (say|include) (message|warning|error) "(.*?)"$/) do |exactness, level, txt|
   ...
   if exactness == 'say'
      ...
      expect(@result[lvl]).to include(txt)
   end
end

Then(/^it should return (\d+) (message|warning|error)s?$/) do |number, type|
